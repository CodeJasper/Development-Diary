...
import { EditorButtonConfiguration, EditorButtonName, ExtensionConfigurations, HeadingLevel, HeadingState } from "@/components/TextEditor/types";
...
export default function useEditorButtonsState(props: useEditorButtonsStateProps) {
  const { editor, buttonNames, extensionConfigurations } = props;
  
  const getHeadingConfigurations = () => {
    ...
    HEADING_LEVELS.forEach((headingLevel: HeadingLevel ) => {
      if(buttonNames.find((name) => name === `heading${headingLevel}`)) {
        headingConfiurations.push({
          ...
          command: () => editor.chain().focus().toggleHeading({ level: headingLevel }).run(),
          isActive: editorState[`isHeading${headingLevel}` as keyof typeof editorState],
        })
      }
    })
