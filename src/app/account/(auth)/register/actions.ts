...
import type { RegistrationFormProps } from "./page";
...
export async function registerUser(
	_: FormState<RegistrationFormProps>,
	data: FormData,
) {
	...
	if (existing?.email === email) {
		return {
			generalErrors: ["Este correo ya está registrado."],
			data: { email, userName },
		};
	}
	...
	if (existing?.userName === userName) {
		return {
			generalErrors: ["Este nombre de usuario ya está registrado."],
			data: { email, userName },
		};
	}
